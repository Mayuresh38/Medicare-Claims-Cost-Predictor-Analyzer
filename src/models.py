...
def build_ann_model(input_dim, learning_rate=0.001, dropout_rate=0.2, units_1=128, units_2=64, units_3=32):
    ...
    Accepts hyperparameters for tuning and uses BatchNormalization and Huber loss.
    """
    inputs = layers.Input(shape=(input_dim,), name="tabular_inputs")
    
    x = layers.Dense(units_1, kernel_regularizer=tf.keras.regularizers.l2(0.01))(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    x = layers.Dropout(dropout_rate)(x)
    
    x = layers.Dense(units_2, kernel_regularizer=tf.keras.regularizers.l2(0.01))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
    x = layers.Dropout(dropout_rate)(x)
    
    x = layers.Dense(units_3, kernel_regularizer=tf.keras.regularizers.l2(0.01))(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation("relu")(x)
